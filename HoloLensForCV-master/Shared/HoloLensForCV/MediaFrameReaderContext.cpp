...
namespace HoloLensForCV
{
        ...
        Windows::Media::Capture::Frames::MediaFrameArrivedEventArgs^ args)
    {
        ...
        if (loc != nullptr) {
            ...
            sensorFrame->orientation = loc->Orientation;
        }
        else {
            sensorFrame->position = Windows::Foundation::Numerics::float3(0,0,0);
            sensorFrame->orientation = Windows::Foundation::Numerics::quaternion(0,0,0,0);
        }
        sensorFrame->locator = _spatialPerception->GetSpatialLocator();
        sensorFrame->coordinateSystem = _spatialPerception->GetOriginFrameOfReference()->CoordinateSystem;
