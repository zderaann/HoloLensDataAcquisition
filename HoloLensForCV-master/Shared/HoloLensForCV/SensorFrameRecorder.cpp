...
namespace HoloLensForCV
{
    ...
    {
        Enable(SensorType::PhotoVideo, ip, port, holospace);

#if ENABLE_HOLOLENS_RESEARCH_MODE_SENSORS
        Enable(SensorType::ShortThrowToFDepth, ip, port, holospace);
        Enable(SensorType::ShortThrowToFReflectivity, ip, port, holospace);
        Enable(SensorType::LongThrowToFDepth, ip, port, holospace);
        Enable(SensorType::LongThrowToFReflectivity, ip, port, holospace);
        Enable(SensorType::VisibleLightLeftLeft, ip, port, holospace);
        Enable(SensorType::VisibleLightLeftFront, ip, port, holospace);
        Enable(SensorType::VisibleLightRightFront, ip, port, holospace);
        Enable(SensorType::VisibleLightRightRight, ip, port, holospace);
    ...
    Windows::Foundation::IAsyncAction^ SensorFrameRecorder::StartAsync()
    {
        return concurrency::create_async(
            [this]()
        {
                Windows::Storage::StorageFolder^ temporaryStorageFolder = //Windows::Storage::KnownFolders::DocumentsLibrary;
