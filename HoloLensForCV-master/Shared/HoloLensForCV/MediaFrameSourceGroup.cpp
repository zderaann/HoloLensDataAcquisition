            ...
            .then([this](Windows::Foundation::Collections::IVectorView<Windows::Media::Capture::Frames::MediaFrameSourceGroup^>^ sourceGroups)
                {
                    ...
                    for (Windows::Media::Capture::Frames::MediaFrameSourceGroup^ sourceGroup : sourceGroups)
                    {
                        ...
                        /* Windows::Foundation::Collections::IVectorView<Windows::Media::Capture::Frames::MediaFrameSourceInfo^>^ sources = sourceGroup->SourceInfos;

                         for (Windows::Media::Capture::Frames::MediaFrameSourceInfo^ source : sources) {
                             const wchar_t*  type = source->MediaStreamType.ToString()->Data();
                             const wchar_t* kind = source->SourceKind.ToString()->Data();
                             const wchar_t* id = source->Id->Data();
                             ...
                             auto descriptions = source->VideoProfileMediaDescription;

                             for (Windows::Media::Capture::MediaCaptureVideoProfileMediaDescription^ profile : descriptions) {
                                 dbg::trace(
                                     L"Profile %d x %d, %f\n", profile->Width, profile->Height, profile->FrameRate);
                                 /*if(source->MediaStreamType == Windows::Media::Capture::MediaStreamType::VideoRecord &&
                                     source->SourceKind == Windows::Media::Capture::Frames::MediaFrameSourceKind::Color &&
                                     )
                             }
                                    ...
                                    createReadersTask = createReadersTask.then([this, startedSensors, source]()
                                        {
                                            ...
                                            //TADY SE VOLI MOD PRO PV CAMERA
                                            if (sensorType == SensorType::PhotoVideo) {
                                                auto formats = source->SupportedFormats;

                                                for (auto it = begin(formats); it != end(formats); it++) {

                                                    if ((*it)->VideoFormat->Width == 1344 && (*it)->VideoFormat->Height == 756 && ceil((double)(*it)->FrameRate->Numerator / (double)(*it)->FrameRate->Denominator) == 30.0) {
                                                        found = it;
                                                        break;
                                                    }


                                                    //dbg::trace( L"Profile %d x %d, %f\n", profile->VideoFormat->Width, profile->VideoFormat->Height, ceil((double)profile->FrameRate->Numerator / (double)profile->FrameRate->Denominator));
...
#if DBG_ENABLE_INFORMATIONAL_LOGGING
            /*if (format->VideoFormat->Width == 1344 && format->VideoFormat->Height == 756 && ceil(format->FrameRate->Numerator/format->FrameRate->Denominator) == 30) {*/
            dbg::trace(
